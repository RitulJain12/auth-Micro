...
async function premiumMiddleware(req,res,next) {
    ...
    //  console.log(req.headers.authorization);
      ...
      req.user=decode;
          ...
          if(!req.user.ispremium) return res.status(403).json({message:"Not a Premium User"});
          if(req.user.premiumEndDate < Date.now()) return res.status(403).json({message:"Premium User Expired"});
       next(); 
    }
    catch(err){
        return res.status(401).json({message:"Unauthorized"});
